...
int copyToFS(FileSystem *fs, void *sourceFileData, unsigned int length, char *destFileName) {
	if (!isFileSystemMounted(fs)) {
		return 0;
	}
	FileRecord file;
	unsigned int blockSize = getOrSetblockSize(0), itr, tmp;
	...
	if (file.numberOfBlocks == 1) {	
		...
		//Read file
		memcpy(buf, sourceFileData, length);
		...
		for (itr = 1; itr <= file.numberOfBlocks; ++itr) {
			memcpy(buf, sourceFileData, (tmp > blockSize) ? blockSize : tmp);
			writeBlock(buf, fileBlocks[itr]);
			setByteVectors(fs, fileBlocks[itr], 0);
			tmp -= blockSize;
			sourceFileData = (char *)sourceFileData + blockSize;
		}
	}
...
void *copyFromFS(FileSystem *fs, char *sourceFileName, unsigned int *length) {
	if (!isFileSystemMounted(fs)) {
		return NULL;
	}
	...
	void *buf, *fileData = (void *)calloc(fs->filetable->files[fileID].size, 1), *fileDataPtr = fileData;	// Trying to keep everything in memory

	if (sourceFile->numberOfBlocks == 1) {
		buf = readBlock(sourceFile->startBlockOrDataBlock);
		memcpy(fileData, buf, fs->filetable->files[fileID].size);
		...
		for (itr = 0; itr < sourceFile->numberOfBlocks; ++itr) {
			...
			memcpy(fileDataPtr, buf, ((bytesToWrite < blockSize) ? bytesToWrite : blockSize));
			bytesToWrite -= ((bytesToWrite < blockSize) ? bytesToWrite : blockSize);
			fileDataPtr = (char *)fileDataPtr + ((bytesToWrite < blockSize) ? bytesToWrite : blockSize);
		}
	}
